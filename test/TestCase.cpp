...
bool TestCase::isTestFilename(boost::filesystem::path const& _filename)
{
	...
	// NOTE: .asmjson and .ethdebugjson rather than .json because JSON files that do not represent test cases exist in some test dirs.
	return (extension == ".sol" || extension == ".yul" || extension == ".asm" || extension == ".asmjson" || extension == ".ethdebugjson" || extension == ".stack") &&
