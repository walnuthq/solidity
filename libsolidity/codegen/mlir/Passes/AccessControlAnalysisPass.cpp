...
class AccessControlAnalysisPass : public mlir::PassWrapper<AccessControlAnalysisPass, mlir::OperationPass<mlir::ModuleOp>> {
public:
...
private:
    ...
    bool hasAccessControlPattern(mlir::Operation* op) {
        ...
        for (auto* user : op->getUsers()) {
            ...
            if (userName.contains("cmp") || userName.contains("eq") ||
                userName.contains("compare") || userName.contains("==")) {
                return true;
            }
        }
...
std::unique_ptr<mlir::Pass> createAccessControlAnalysisPass() {
    return std::make_unique<AccessControlAnalysisPass>();
}
