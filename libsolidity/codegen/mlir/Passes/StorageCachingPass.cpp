...
public:
        ...
        module.walk([&](mlir::Operation* op) {
            ...
            if (op->getName().getStringRef() == "solidity.func") {
                ...
                op->walk([&](mlir::scf::ForOp forOp) {
                    optimizeForLoop(forOp);
                });
                
                op->walk([&](mlir::scf::WhileOp whileOp) {
                    optimizeWhileLoop(whileOp);
                });
            }
        });
    }
    ...
    StringRef getDescription() const final { 
        return "Cache storage variables in loops to reduce gas costs"; 
    }
...
private:
    ...
    bool optimizeForLoop(mlir::scf::ForOp forOp) {
        ...
        forOp.getBody()->walk([this, &storageOps](mlir::Operation* op) {
            analyzeStorageOp(op, storageOps);
        });
    ...
    bool optimizeWhileLoop(mlir::scf::WhileOp whileOp) {
        ...
        whileOp.getAfter().walk([this, &storageOps](mlir::Operation* op) {
            analyzeStorageOp(op, storageOps);
        });
    ...
    bool transformWhileLoopWithCaching(mlir::scf::WhileOp whileOp, const StorageInfo& info) {
        ...
        auto newWhileOp = builder.create<mlir::scf::WhileOp>(
            loc,
            oldResultTypes,
            oldInits
        );
        ...
        for (auto& op : oldBefore.front().getOperations()) {
            if (auto condOp = dyn_cast<mlir::scf::ConditionOp>(op)) {
                ...
                builder.create<mlir::scf::ConditionOp>(
                    condOp.getLoc(),
                    beforeMapping.lookupOrDefault(condOp.getCondition()),
                    condOperands
                );
            } else {
...
std::unique_ptr<mlir::Pass> createStorageCachingPass() {
    return std::make_unique<StorageCachingPass>();
}
